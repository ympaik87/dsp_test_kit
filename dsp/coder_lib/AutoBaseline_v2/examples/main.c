...
#include "main.h"
#include "AutoBaseline_v2_terminate.h"
#include "AutoBaseline_v2_initialize.h"

/* Function Declarations */
static void argInit_d100x1_real_T(double result_data[], int result_size[1]);
static double argInit_real_T(void);
static void main_AutoBaseline_v2(void);
...
static void main_AutoBaseline_v2(void)
{
  ...
  /* Initialize function 'AutoBaseline_v2' input arguments. */
  ...
  /* Call the entry-point 'AutoBaseline_v2'. */
  AutoBaseline_v2(x_data, x_size, RD_data, RD_size, perturbation_data,
                  perturbation_size, SFC_tmp, SFC_tmp, DRFU, Thrd, RPC, RC, dfM,
                  dfC, PMC, FB, AR, isPC, ver_array_data, ver_array_size,
                  argInit_real_T(), &result_well, &DataProcessNum,
                  DataProcess_data, DataProcess_size, &LSR_val, RD_diff_data,
                  RD_diff_size, ivd_cdd_data, ivd_cdd_size, cff, scd_fit_data,
                  scd_fit_size, &R_p2, &EFC, ABSD_orig_data, ABSD_orig_size,
                  ABSD_data, ABSD_size, f_data, f_size, f_new_data, f_new_size,
                  &sht, &sht2, param, param_new, &R2, &EndRFU, &df);
}
