...
void AutoBaseline_v2(const double x_data[], const int x_size[1], double RD_data[],
                     ...
                     ver_array_data[], const int ver_array_size[1], double DS,
  ...
  *EndRFU = RD_data[RD_size[0] - 1] - RD_scale;
  ...
  if (*result_well == 0.0) {
    if (ver_array_data[3] == 3.0) {
      ...
      if (ver_array_data[3] == 4.0) {
        Coder_Section_Jumping_v4_DoN(RD_data, RD_size, DRFU, result_well,
          DataProcessNum, DS, FB, SFC, HTC, RD_size[0], RD_diff_data,
          RD_diff_size, ivd_cdd_data, ivd_cdd_size, &RD_scale);
      }
    }
    ...
    *EndRFU = RD_data[varargin_1 - 1] - RD_scale;
    if (neg_BL == 1) {
      *DataProcessNum = 0.0;
      *result_well = 0.0;
    }
  ...
  if (*result_well == 0.0) {
    ...
    if (ver_array_data[0] == 1.0) {
      /*          ABSD=Coder_ABSD_corrector(SFC,MFC,ABSD); */
      RD_scale = Coder_ABSD_corrector_v1_1(SFC, MFC, ABSD_data, ABSD_size);
    } else if (ver_array_data[0] == 0.0) {
      /*          ABSD=Coder_ABSD_corrector_v3(SFC,MFC,ABSD,Thrd,RD_diff,TC); */
    ...
    b_ABSD_orig_size[0] = ABSD_size[0];
    idx = ABSD_size[0];
    for (i0 = 0; i0 < idx; i0++) {
      b_RD_data[i0] = ABSD_orig_data[i0] - ABSD_data[i0];
    }

    b_abs(b_RD_data, b_ABSD_orig_size, b_tmp_data, tmp_size);
  ...
  if ((*result_well == 1.0) || (*result_well == 2.0)) {
    ...
    if ((*EFC < varargin_1) && (RD_data[varargin_1 - 1] < RD_data[0]) && (cff[0]
      ...
      if ((apnd - scd_fit_data[varargin_1 - 1]) / (RD_data[varargin_1 - 1] -
        ...
        for (i1 = 0; i1 <= cdiff; i1++) {
          d_tmp_data[i1] = (signed char)((signed char)((signed char)i0 + (signed
            char)i1) - 1);
        }

        idx = cdiff + 1;
        for (i0 = 0; i0 < idx; i0++) {
          scd_fit_data[d_tmp_data[i0]] = cff[0] * c_y->data[i0] + cff[1];
        }
      }
    }
...
/* End of code generation (AutoBaseline_v2.c) */
