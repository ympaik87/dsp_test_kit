/*
 * Coder_DistanceBetweenSegs_v2.c
 *
 * Code generation for function 'Coder_DistanceBetweenSegs_v2'
 *
 */
...
void Coder_DistanceBetweenSegs_v2(const double RD_data[], const int RD_size[1],
  double SC, double EC, double *d2, double *slope)
{
  int loop_ub;
  int i12;
  emxArray_real_T *y;
  ...
  for (i12 = 0; i12 < loop_ub; i12++) {
    RD_new_data[i12] = RD_data[i12] - RD_data[(int)SC - 1];
  }
  ...
  *slope = b_slope_tmp / slope_tmp;
  ...
  if (rtIsNaN(EC)) {
    ...
    b_y->size[0] = 1;
    b_y->size[1] = 1;
    emxEnsureCapacity_real_T(b_y, i12);
    b_y->data[0] = rtNaN;
  } else if (EC < SC) {
    b_y->size[0] = 1;
    b_y->size[1] = 0;
  ...
  i12 = c_y->size[0];
  c_y->size[0] = y->size[1];
  emxEnsureCapacity_real_T(c_y, i12);
  loop_ub = y->size[1];
  for (i12 = 0; i12 < loop_ub; i12++) {
    c_y->data[i12] = y->data[i12];
  }
  ...
  emxEnsureCapacity_real_T(d_y, i12);
  loop_ub = b_y->size[1];
  for (i12 = 0; i12 < loop_ub; i12++) {
    d_y->data[i12] = b_y->data[i12];
  }

  emxFree_real_T(&b_y);
  loop_ub = c_y->size[0];
  for (i12 = 0; i12 < loop_ub; i12++) {
    segment_data[i12] = slope_tmp * (d_y->data[i12] - SC) + RD_new_data[(int)SC
      - 1];
  }
  ...
  for (k = 0; k < loop_ub; k++) {
    c_y->data[k] = RD_new_data[i12 + k] - segment_data[k];
  }
  ...
  *d2 = fabs((RD_new_data[nm1d2 - 1] - *slope * (double)nm1d2) -
             (RD_new_data[min_cycle - 1] - *slope * (double)min_cycle)) / sqrt
    (*slope * *slope + 1.0);
}

/* End of code generation (Coder_DistanceBetweenSegs_v2.c) */
