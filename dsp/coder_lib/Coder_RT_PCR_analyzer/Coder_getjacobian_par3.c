  ...
  int x_size[1], const double fixed_pt[2], double J_data[], int J_size[2])
{
  ...
  for (i29 = 0; i29 < loop_ub; i29++) {
    Cmm1_tmp_data[i29] = fixed_pt[0] - x_data[i29];
  }

  param_size[0] = x_size[0];
  loop_ub = x_size[0];
  for (i29 = 0; i29 < loop_ub; i29++) {
    param_data[i29] = param[3] * Cmm1_tmp_data[i29];
  }
  ...
  loop_ub = Cmm1_size[0];
  for (i29 = 0; i29 < loop_ub; i29++) {
    Cmm2_data[i29] = 1.0 + Cmm1_data[i29];
  }

  loop_ub = Cmm2_size[0];
  for (i29 = 0; i29 < loop_ub; i29++) {
    J_data[i29] = 1.0 - 2.0 / Cmm2_data[i29];
  }
  ...
  loop_ub = x_size[0];
  for (i29 = 0; i29 < loop_ub; i29++) {
    tmp_data[i29] = d10 * Cmm1_tmp_data[i29] * Cmm1_data[i29];
  }
  ...
  emxFree_real_T(&r4);
  for (i29 = 0; i29 < loop_ub; i29++) {
    J_data[i29 + J_size[0]] = param_data[i29];
  }
}
