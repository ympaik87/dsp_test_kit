  ...
  while ((!exitg1) && ((ERR < 0.5) || (ERR > 1.5) || (ERR2 < 0.5) || (state ==
    ...
    if (EC < floor((*SC + TC) / 2.0)) {
      *result = 3.0;
      ...
      x_size_tmp = ABSD_size_tmp - i78;
      b_x_size[0] = x_size_tmp;
      for (ABSD_size_tmp = 0; ABSD_size_tmp < x_size_tmp; ABSD_size_tmp++) {
        b_x_data[ABSD_size_tmp] = x_data[i78 + ABSD_size_tmp];
      }

      ABSD_size_tmp = i80 - i79;
      ABSD_size[0] = ABSD_size_tmp;
      for (i78 = 0; i78 < ABSD_size_tmp; i78++) {
        b_ABSD_data[i78] = ABSD_data[i79 + i78];
      }
      ...
      if (fabs(param[3]) > 2.0) {
        ...
        for (i78 = 0; i78 < ABSD_size_tmp; i78++) {
          b_ABSD_data[i78] = ABSD_data[i79 + i78] + perturbation_data[i81 + i78];
        }
