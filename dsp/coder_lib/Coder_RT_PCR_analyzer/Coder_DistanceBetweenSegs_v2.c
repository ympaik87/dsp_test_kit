  ...
  double SC, double EC, double *d2, double *slope)
{
  ...
  for (i6 = 0; i6 < loop_ub; i6++) {
    RD_new_data[i6] = RD_data[i6] - RD_data[(int)SC - 1];
  }
  ...
  *slope = b_slope_tmp / slope_tmp;
  ...
  i6 = c_y->size[0];
  c_y->size[0] = y->size[1];
  emxEnsureCapacity_real_T(c_y, i6);
  loop_ub = y->size[1];
  for (i6 = 0; i6 < loop_ub; i6++) {
    c_y->data[i6] = y->data[i6];
  }
  ...
  emxEnsureCapacity_real_T(d_y, i6);
  loop_ub = b_y->size[1];
  for (i6 = 0; i6 < loop_ub; i6++) {
    d_y->data[i6] = b_y->data[i6];
  }

  emxFree_real_T(&b_y);
  loop_ub = c_y->size[0];
  for (i6 = 0; i6 < loop_ub; i6++) {
    segment_data[i6] = slope_tmp * (d_y->data[i6] - SC) + RD_new_data[(int)SC -
      1];
  }
  ...
  for (k = 0; k < loop_ub; k++) {
    c_y->data[k] = RD_new_data[i6 + k] - segment_data[k];
  }
