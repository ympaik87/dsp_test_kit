  ...
  *sht2 = 0.0;
  ...
  if ((param3 > SFC) && (param3 < TC - 2.0) && (EFC0 < TC - 2.0)) {
    ...
    if (EFC0 + 2.0 < EFC0 - 2.0) {
      ...
      check_itv->size[1] = 0;
                ...
                EFC0 + 2.0)) {
      i86 = check_itv->size[0] * check_itv->size[1];
      check_itv->size[0] = 1;
      check_itv->size[1] = 1;
      emxEnsureCapacity_real_T(check_itv, i86);
      ...
      emxEnsureCapacity_real_T(check_itv, i86);
      for (i86 = 0; i86 <= nm1d2; i86++) {
        check_itv->data[i86] = (EFC0 - 2.0) + (double)i86;
      }
    } else {
    ...
    ndbl = f_data[0];
    i86 = varargin_1->size[0];
    varargin_1->size[0] = check_itv->size[1];
    emxEnsureCapacity_real_T(varargin_1, i86);
    nm1d2 = check_itv->size[1];
    for (i86 = 0; i86 < nm1d2; i86++) {
      varargin_1->data[i86] = ((f_data[(int)check_itv->data[i86] - 1] -
        ABSD_data[(int)check_itv->data[i86] - 1]) - ndbl) + SRFU;
    }
    ...
    if (varargin_1->size[0] <= 2) {
      ...
      if (nm1d2 == 0) {
        ...
        i86 = nm1d2 + 1;
        for (num_RFU_EC = i86; num_RFU_EC <= n; num_RFU_EC++) {
    ...
    if ((check_val > 0.3) || ((ndbl > 0.8) && (ndbl < 5.0))) {
      ...
      while ((RFU_EC - SRFU > Thrd) && (EC > SFC) && (DBorASMM > 1.5) && (i_ASMM
        ...
        for (i86 = 0; i86 < 5; i86++) {
          check_itv2[i86] = ABSD_data[(int)(EC + (-2.0 + (double)i86)) - 1];
        }

        Coder_local_smoothing(check_itv2, f);
        for (nm1d2 = 0; nm1d2 < 5; nm1d2++) {
          b_check_itv2[nm1d2] = check_itv2[nm1d2] - f[nm1d2];
        }

        c_diff(b_check_itv2, b_SRFU);
            ...
            if (ndbl == 0.0) {
              ...
              if (ndbl == 0.0) {
                ...
                if ((i_DB == 1) && (1 + i_RFU_EC == 1) && (check_val > 0.3)) {
                  ...
                  ABSD_size_tmp = i87 - i86;
                  c_ABSD_size[0] = ABSD_size_tmp;
                  for (i87 = 0; i87 < ABSD_size_tmp; i87++) {
                    c_ABSD_data[i87] = ABSD_data[i86 + i87] - f_left_data[i89 +
                      i87];
                  }

                  ABSD_size_tmp = i91 - i90;
                  d_ABSD_size[0] = ABSD_size_tmp;
                  for (i86 = 0; i86 < ABSD_size_tmp; i86++) {
                    d_ABSD_data[i86] = ABSD_data[i90 + i86];
                  }
                  ...
                  for (i87 = 0; i87 < ABSD_size_tmp; i87++) {
                    b_ABSD_data[i87] = ABSD_data[i86 + i87] - f_right_data[i89 +
                      i87];
                  }
                  ...
                  for (i86 = 0; i86 < ABSD_size_tmp; i86++) {
                    f_left_data[i86] = ABSD_data[i90 + i86];
                  }
      ...
      if ((*result_well == 0.0) && (RFU_EC_save - SRFU > Thrd) && (EC_save > SFC))
      {
        i86 = (int)EC_save;
        i87 = i86 - 1;
        f_data[i87] = (f_new_data[(int)(EC_save - 1.0) - 1] + f_new2_data[(int)
                       (EC_save + 1.0) - 1]) / 2.0;
        if (1.0 > EC_save - 1.0) {
          i88 = 0;
        } else {
        ...
        for (i90 = 0; i90 <= nm1d2; i90++) {
          f_data[(i91 + i90) - 1] = f_new2_data[(i89 + i90) - 1];
        }

        *sht = f_data[0];
        if ((DBorASMM > 1.5) && (EC_save > 4.0) && (EC_save + 3.0 < TC) &&
            (f_data[i86 - 3] - f_data[0] > Thrd)) {
          f_temp[0] = e_mean(*(double (*)[5])&f_data[-5 + i86]);
          f_temp[1] = e_mean(*(double (*)[5])&f_data[-4 + i86]);
          for (i89 = 0; i89 < 5; i89++) {
            ndbl = EC_save + (-2.0 + (double)i89);
            f[i89] = f_data[(int)ndbl - 1];
            check_itv2[i89] = ndbl;
          }

          f_temp[2] = e_mean(f);
          f_temp[3] = e_mean(*(double (*)[5])&f_data[-2 + i86]);
          f_temp[4] = e_mean(*(double (*)[5])&f_data[i87]);
          for (i86 = 0; i86 < 5; i86++) {
            f_data[(int)check_itv2[i86] - 1] = f_temp[i86];
          }
        }
  ...
  if ((*DataProcessNum == 0.0) && ((SRFU - f_data[0]) / (f_data[f_size[0] - 1] -
    ...
    if ((cdiff == 0.0) && (f_new_data[(int)TC - 1] - f_new_data[0] >
      ...
      i86 = (int)MFC - 1;
      if ((f_new_data[i86] - f_data[i86]) / (f_new_data[0] - f_data[0]) < 0.9) {
        *result_well = -2.0;
        *DataProcessNum = -2.0;
        *sht2 = f_new_data[0];
