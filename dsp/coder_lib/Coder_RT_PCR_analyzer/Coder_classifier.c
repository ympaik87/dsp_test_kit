                      ...
                      *EndRFU, double *df)
{
  ...
  for (n = 0; n < ABSD_size_tmp; n++) {
    Dev_f_data[n] = ABSD_data[i62 + n] - f_data[idx + n];
  }

  ABSD_size_tmp = i63 - k;
  c_ABSD_size[0] = ABSD_size_tmp;
  for (i62 = 0; i62 < ABSD_size_tmp; i62++) {
    b_ABSD_data[i62] = ABSD_data[k + i62];
  }
  ...
  *EndRFU = f_data[(int)TC - 1] - f_data[0];
  ...
  i62 = varargin_1->size[0];
  varargin_1->size[0] = c_ABSD_size[0];
  emxEnsureCapacity_real_T(varargin_1, i62);
  n = c_ABSD_size[0];
  for (i62 = 0; i62 < n; i62++) {
    varargin_1->data[i62] = b_ABSD_data[i62];
  }
