  ...
  ABSD_size[1], double Thrd, const double RD_diff_data[], double TC)
{
  ...
  n = varargin_1->size[0];
  loop_ub = (int)(SFC - 1.0);
  varargin_1->size[0] = (loop_ub + k) - i58;
  ...
  n = k - i58;
  for (k = 0; k < n; k++) {
    varargin_1->data[k + loop_ub] = ABSD_data[i58 + k];
  }
  ...
  corr_part_size[0] = n;
  for (k = 0; k < n; k++) {
    corr_part_data[k] = ABSD_data[i58 + k];
  }
    ...
    if (amp_ratio > 0.2) {
      ...
      if (scale_corr_part / Thrd > 0.5) {
        ...
        if (LSR > 2.5) {
          ...
          c_ABSD_size[0] = b_idx;
          for (k = 0; k < b_idx; k++) {
            corr_part_data[k] = ABSD_data[i58 + k] + RD_diff_data[n + k];
          }
