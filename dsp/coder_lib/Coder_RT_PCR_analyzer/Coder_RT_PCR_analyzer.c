/*
 * Coder_RT_PCR_analyzer.c
 *
 * Code generation for function 'Coder_RT_PCR_analyzer'
 *
 */
...
void Coder_RT_PCR_analyzer(const double x_data[], const int x_size[1], double
  ...
  ivd_cdd_ouput_data[], int ivd_cdd_ouput_size[2], double cff[2], double
  scd_fit_data[], int scd_fit_size[1], double *R_p2, double *EFC, double
  ABSD_orig_data[], int ABSD_orig_size[1], double ABSD_data[], int ABSD_size[1],
  double f_data[], int f_size[1], double f_new_data[], int f_new_size[1], double
  *sht, double *sht2, double param[4], double param_new[4], double *R2, double
  *EndRFU, double *df)
{
  int idx;
  int ivd_cdd_size[2];
  ...
  *EndRFU = RD_data[RD_size[0] - 1] - ex;
  ...
  if (*result_well == 0.0) {
    ...
    loop_ub_tmp = idx - i0;
    for (idx = 0; idx < loop_ub_tmp; idx++) {
      varargin_1_data[idx] = b_RD_data[i0 + idx];
    }
    ...
    if (loop_ub_tmp <= 2) {
      if (loop_ub_tmp == 1) {
        ...
        ex = b_RD_data[i0 + 1];
        if ((b_RD_data[i0] > ex) || (rtIsNaN(b_RD_data[i0]) && (!rtIsNaN(ex))))
        {
        } else {
          ex = b_RD_data[i0];
        }
    ...
    *EndRFU = b_RD_data[varargin_1 - 1] - ex;
  }
  ...
  if (*result_well == 0.0) {
    *result_well = 0.0;
    Coder_Section_QuickAmp_v3(x_data, x_size, RD_data, RD_size, SFC, EFC, HTC,
      varargin_1, result_well, AR, FB, &EFC0, cff, scd_fit_data, scd_fit_size,
      ABSD_data, ABSD_size, ABSD_orig_data, ABSD_orig_size, R_p2, &SRFU);

    /*      [EFC, EFC0, cff, scd_fit, ABSD, ABSD_orig, R_p2, SRFU, result_well]=Coder_Section_QuickAmp_v4(x,RD,SFC,EFC,HTC,TC,result_well,1,FB,500); */
  }
  ...
  if (*result_well == 0.0) {
    ...
    /*      [EFC, EFC0, cff, scd_fit]=Coder_Section_Baseline_v6(x,RD,SFC,MFC,EFC,TC,DRFU,LinearOrNot,AR,FB,dfC); */
    idx = scd_fit_size[0];
    for (i0 = 0; i0 < idx; i0++) {
  ...
  if (*result_well == 0.0) {
    ...
    if (ABSD_corrector_ver == 1.0) {
      Coder_ABSD_corrector(SFC, MFC, ABSD_data, ABSD_size);
    } else {
      if (ABSD_corrector_ver == 0.0) {
  ...
  if ((*result_well == 0.0) && (UseStrangePatternCorrector != 0.0)) {
...
/* End of code generation (Coder_RT_PCR_analyzer.c) */
