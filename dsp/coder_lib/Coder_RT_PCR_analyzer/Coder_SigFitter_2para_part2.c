  ...
  double fixed_pt[2], double param[4], double *iter, double *state)
{
  ...
  *state = 1.0;
  ...
  r_size[0] = y_size[0];
  aoffset = y_size[0];
  for (i27 = 0; i27 < aoffset; i27++) {
    r_data[i27] = y_data[i27] - r_data[i27];
  }
  ...
  *iter = 1.0;
  ...
  while ((!exitg1) && (b_iter < 200)) {
    *iter = 1.0 + (double)b_iter;
    ...
    for (i27 = 0; i27 < aoffset; i27++) {
      inner = i27 << 1;
      a_tmp_data[inner] = J_data[i27];
      a_tmp_data[1 + inner] = J_data[i27 + J_size[0]];
    }
    ...
    if (J_size[0] == 1) {
      ...
      for (k = 0; k < inner; k++) {
        i27 = k << 1;
        sse = J_data[inner + k];
        J2[2] += sse * a_tmp_data[i27];
        J2[3] += sse * a_tmp_data[i27 + 1];
      }
    }
    ...
    if ((J_size[0] == 1) || (r_size[0] == 1)) {
      ...
      sse = 0.0;
      for (i27 = 0; i27 < aoffset; i27++) {
        sse += a_tmp_data[i27 << 1] * r_data[i27];
      }

      y_idx_1 = 0.0;
      for (i27 = 0; i27 < aoffset; i27++) {
        y_idx_1 += a_tmp_data[1 + (i27 << 1)] * r_data[i27];
      }
    } else {
      inner = J_size[0];
      sse = 0.0;
      ...
      while ((!exitg2) && (sse > sse_old)) {
        ...
        if (lambda > 1.0E+16) {
          ...
          if ((a_tmp_size_idx_1 == 1) || (r_size[0] == 1)) {
            sse = 0.0;
            for (i27 = 0; i27 < a_tmp_size_idx_1; i27++) {
              sse += a_tmp_data[i27 << 1] * r_data[i27];
            }
