  ...
  double SC, double EC, double *d2, double *slope)
{
  ...
  for (i8 = 0; i8 < loop_ub; i8++) {
    RD_new_data[i8] = RD_data[i8] - RD_data[(int)SC - 1];
  }
  ...
  *slope = b_slope_tmp / slope_tmp;
  ...
  i8 = c_y->size[0];
  c_y->size[0] = y->size[1];
  emxEnsureCapacity_real_T(c_y, i8);
  loop_ub = y->size[1];
  for (i8 = 0; i8 < loop_ub; i8++) {
  ...
  emxEnsureCapacity_real_T(d_y, i8);
  loop_ub = b_y->size[1];
  for (i8 = 0; i8 < loop_ub; i8++) {
    d_y->data[i8] = b_y->data[i8];
  }

  emxFree_real_T(&b_y);
  loop_ub = c_y->size[0];
  for (i8 = 0; i8 < loop_ub; i8++) {
    segment_data[i8] = slope_tmp * (d_y->data[i8] - SC) + RD_new_data[(int)SC -
      1];
  }
  ...
  for (k = 0; k < loop_ub; k++) {
    c_y->data[k] = RD_new_data[i8 + k] - segment_data[k];
  }
