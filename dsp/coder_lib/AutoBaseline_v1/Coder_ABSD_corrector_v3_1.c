  ...
  ABSD_size[1], double Thrd, const double RD_diff_data[], double TC)
{
  ...
  n = varargin_1->size[0];
  loop_ub = (int)(SFC - 1.0);
  varargin_1->size[0] = (loop_ub + k) - i42;
  ...
  n = k - i42;
  for (k = 0; k < n; k++) {
    varargin_1->data[k + loop_ub] = ABSD_data[i42 + k];
  }
  ...
  corr_part_size[0] = n;
  for (k = 0; k < n; k++) {
    corr_part_data[k] = ABSD_data[i42 + k];
  }
  ...
  if ((var(b_ABSD_data, b_ABSD_size) / var(ABSD_data, ABSD_size) < 0.05) && (LSR
    ...
    if (amp_ratio > 0.2) {
      ...
      if (scale_corr_part / Thrd > 0.5) {
        ...
        if (LSR > 2.5) {
          ...
          d_ABSD_size[0] = b_idx;
          for (k = 0; k < b_idx; k++) {
            b_ABSD_data[k] = ABSD_data[i42 + k] + RD_diff_data[n + k];
          }
    ...
    if (scale_corr_part / b_ex > amp_ratio) {
      ...
      b_Coder_local_smoothing(b_ABSD_data, c_ABSD_size, corr_part_data,
        corr_part_size);
      for (i42 = 0; i42 < idx; i42++) {
        d_tmp_data[i42] = i42;
      }

      for (i42 = 0; i42 < idx; i42++) {
        b_ABSD_data[i42] = (ABSD_data[i42] - corr_part_data[i42]) + ex;
      }

      for (i42 = 0; i42 < idx; i42++) {
        ABSD_data[d_tmp_data[i42]] = b_ABSD_data[i42];
      }
