                  ...
                  int nq, emxArray_int32_T *iwork, emxArray_real_T *xwork)
{
  ...
  if (nq != 0) {
    ...
    for (iout = 0; iout < n_tmp; iout++) {
      i31 = offset + iout;
      iwork->data[iout] = idx->data[i31];
      xwork->data[iout] = x->data[i31];
    }
      ...
      if (xwork->data[p] <= xwork->data[q]) {
        ...
        if (q + 1 < n_tmp) {
          ...
          for (iout = p + 1; iout <= np; iout++) {
            i31 = q + iout;
            idx->data[i31] = iwork->data[iout - 1];
            x->data[i31] = xwork->data[iout - 1];
          }
...
void sortIdx(emxArray_real_T *x, emxArray_int32_T *idx)
{
  ...
  i30 = idx->size[0];
  idx->size[0] = i1;
  emxEnsureCapacity_int32_T(idx, i30);
  for (i30 = 0; i30 < i1; i30++) {
    idx->data[i30] = 0;
  }
  ...
  if (x->size[0] != 0) {
    ...
    i30 = iwork->size[0];
    iwork->size[0] = i1;
    emxEnsureCapacity_int32_T(iwork, i30);
    for (i30 = 0; i30 < i1; i30++) {
      iwork->data[i30] = 0;
    }
    ...
    i30 = xwork->size[0];
    xwork->size[0] = i1;
    emxEnsureCapacity_real_T(xwork, i30);
    for (i30 = 0; i30 < i1; i30++) {
      xwork->data[i30] = 0.0;
    }
    ...
    for (k = 0; k < b_n; k++) {
      if (rtIsNaN(x->data[k])) {
        ...
        if (ib + 1 == 4) {
          ...
          i30 = perm[0] - 1;
          idx->data[quartetOffset - 3] = idx4[i30];
          i4 = perm[1] - 1;
                ...
                do {
                  ...
                  if (b_xwork[i4] <= b_xwork[i1]) {
                    ...
                    if (i1 + 1 < bLen2) {
                      ...
                      for (i1 = i4 + 1; i1 <= n; i1++) {
                        i30 = ib + i1;
                        idx->data[i30] = b_iwork[i1 - 1];
                        x->data[i30] = b_xwork[i1 - 1];
                      }
