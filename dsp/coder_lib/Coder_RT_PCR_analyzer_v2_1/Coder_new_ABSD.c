                    ...
                    double *result_well)
{
  ...
  *result_well = -1.0;
  ...
  for (i74 = 0; i74 < loop_ub; i74++) {
    cNdiff_data[i74] = ABSD_data[i74] - ((cc_idx_0 * x_data[i74] * x_data[i74] +
      cc_idx_1 * x_data[i74]) + cc_idx_2);
  }
  ...
  for (i74 = 0; i74 < ii_size_idx_1; i74++) {
    cp_data[i74] = ii_data[i74];
  }
    ...
    if (((double)idx - 1.0) - 2.0 * apnd < 1.0) {
      *result_well = 0.0;
      ...
      if (cp_data[(int)(((double)idx - 1.0) - 2.0 * apnd) - 1] > cp_data[(int)
        ...
        i74 = (int)cp_data[(int)(((double)idx - 1.0) - 2.0 * apnd) - 1] - 1;
        ii_size_idx_1 = (int)cp_data[(int)((double)idx - 2.0 * apnd) - 1];
      }

      ii = y->size[0] * y->size[1];
      y->size[0] = 1;
      loop_ub = ii_size_idx_1 - i74;
      y->size[1] = loop_ub;
      emxEnsureCapacity_real_T(y, ii);
      for (ii_size_idx_1 = 0; ii_size_idx_1 < loop_ub; ii_size_idx_1++) {
        y->data[ii_size_idx_1] = cNdiff_data[i74 + ii_size_idx_1];
      }

      n = loop_ub - 1;
      if (loop_ub <= 2) {
        if (loop_ub == 1) {
          cdiff = cNdiff_data[i74];
          nx = 1;
        } else {
          cdiff = cNdiff_data[i74 + 1];
          if ((cNdiff_data[i74] > cdiff) || (rtIsNaN(cNdiff_data[i74]) &&
               (!rtIsNaN(cdiff)))) {
            nx = 2;
          } else {
            cdiff = cNdiff_data[i74];
        ...
        if (nx == 0) {
          ...
          cdiff = cNdiff_data[(i74 + nx) - 1];
          i74 = nx + 1;
          for (ii = i74; ii <= n + 1; ii++) {
  ...
  if (*result_well == -1.0) {
    ...
    loop_ub = (signed char)i74 - 1;
    for (ii_size_idx_1 = 0; ii_size_idx_1 <= loop_ub; ii_size_idx_1++) {
      b_tmp_data[ii_size_idx_1] = (signed char)ii_size_idx_1;
    }

    cc_idx_0 = ABSD_data[(int)*EFC - 1];
    loop_ub = (signed char)i74;
    for (i74 = 0; i74 < loop_ub; i74++) {
      ABSD_data[b_tmp_data[i74]] = cc_idx_0;
    }
  }
}
