                  ...
                  int nq, emxArray_int32_T *iwork, emxArray_real_T *xwork)
{
  ...
  if (nq != 0) {
    ...
    for (iout = 0; iout < n_tmp; iout++) {
      i47 = offset + iout;
      iwork->data[iout] = idx->data[i47];
      xwork->data[iout] = x->data[i47];
    }
      ...
      if (xwork->data[p] <= xwork->data[q]) {
        ...
        if (q + 1 < n_tmp) {
          ...
          for (iout = p + 1; iout <= np; iout++) {
            i47 = q + iout;
            idx->data[i47] = iwork->data[iout - 1];
            x->data[i47] = xwork->data[iout - 1];
          }
...
void sortIdx(emxArray_real_T *x, emxArray_int32_T *idx)
{
  ...
  i46 = idx->size[0];
  idx->size[0] = i1;
  emxEnsureCapacity_int32_T(idx, i46);
  for (i46 = 0; i46 < i1; i46++) {
    idx->data[i46] = 0;
  }
  ...
  if (x->size[0] != 0) {
    ...
    i46 = iwork->size[0];
    iwork->size[0] = i1;
    emxEnsureCapacity_int32_T(iwork, i46);
    for (i46 = 0; i46 < i1; i46++) {
      iwork->data[i46] = 0;
    }
    ...
    i46 = xwork->size[0];
    xwork->size[0] = i1;
    emxEnsureCapacity_real_T(xwork, i46);
    for (i46 = 0; i46 < i1; i46++) {
      xwork->data[i46] = 0.0;
    }
    ...
    for (k = 0; k < b_n; k++) {
      if (rtIsNaN(x->data[k])) {
        ...
        if (ib + 1 == 4) {
          ...
          i46 = perm[0] - 1;
          idx->data[quartetOffset - 3] = idx4[i46];
          i4 = perm[1] - 1;
                ...
                do {
                  ...
                  if (b_xwork[i4] <= b_xwork[i1]) {
                    ...
                    if (i1 + 1 < bLen2) {
                      ...
                      for (i1 = i4 + 1; i1 <= n; i1++) {
                        i46 = ib + i1;
                        idx->data[i46] = b_iwork[i1 - 1];
                        x->data[i46] = b_xwork[i1 - 1];
                      }
