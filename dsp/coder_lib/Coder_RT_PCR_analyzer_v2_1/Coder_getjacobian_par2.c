  ...
  int x_size[1], const double fixed_pt[2], double J_data[], int J_size[2])
{
  ...
  for (i28 = 0; i28 < loop_ub; i28++) {
    Cmm1_tmp_data[i28] = param[2] - x_data[i28];
  }

  param_size[0] = x_size[0];
  loop_ub = x_size[0];
  for (i28 = 0; i28 < loop_ub; i28++) {
    param_data[i28] = param[3] * Cmm1_tmp_data[i28];
  }
  ...
  loop_ub = tmp_size[0];
  for (i28 = 0; i28 < loop_ub; i28++) {
    Cmm2_data[i28] = 1.0 + Cmm1_data[i28];
  }
  ...
  loop_ub = tmp_size[0];
  for (i28 = 0; i28 < loop_ub; i28++) {
    param_data[i28] = d8 * (Cmm3 - Cmm1_data[i28]);
  }
  ...
  loop_ub = Cmm2_size[0];
  for (i28 = 0; i28 < loop_ub; i28++) {
    param_data[i28] = d7 * (Cmm3_tmp * Cmm2_data[i28] - Cmm1_tmp_data[i28] *
      (1.0 + Cmm3) * Cmm1_data[i28]);
  }

  rdivide_helper(param_data, c_tmp_size, J_tmp_data, tmp_data, tmp_size);
  loop_ub = tmp_size[0];
  for (i28 = 0; i28 < loop_ub; i28++) {
    J_data[i28 + J_size[0]] = tmp_data[i28];
  }
}
