  ...
  while ((!exitg1) && ((ERR < 0.5) || (ERR > 1.5) || (ERR2 < 0.5) || (state ==
    ...
    if (*SC > MFC2) {
      *result = 3.0;
      ...
      x_size_tmp = ABSD_size_tmp - i92;
      b_x_size[0] = x_size_tmp;
      for (ABSD_size_tmp = 0; ABSD_size_tmp < x_size_tmp; ABSD_size_tmp++) {
        b_x_data[ABSD_size_tmp] = x_data[i92 + ABSD_size_tmp];
      }

      ABSD_size_tmp = i94 - i93;
      ABSD_size[0] = ABSD_size_tmp;
      for (i92 = 0; i92 < ABSD_size_tmp; i92++) {
        b_ABSD_data[i92] = ABSD_data[i93 + i92];
      }
      ...
      if (fabs(param[3]) > 2.0) {
        ...
        for (i92 = 0; i92 < ABSD_size_tmp; i92++) {
          b_ABSD_data[i92] = ABSD_data[i93 + i92] + perturbation_data[i95 + i92];
        }
