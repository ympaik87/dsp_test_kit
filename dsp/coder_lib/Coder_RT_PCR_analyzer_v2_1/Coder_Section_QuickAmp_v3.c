  ...
  double *SRFU)
{
  ...
  *SRFU = 0.0;
  ...
  if ((RDS > 500.0 * AR) && (FB == 0.0)) {
    /*       if (RDS>700*AR) && (FB==0) */
    i51 = (int)HTC - 1;
    abc = RD_data[i51] - RD_data[RDS_tmp_tmp];
    if (abc > 0.0) {
      abc /= RDS;
      if (abc > 0.7) {
        quick_amp = 1;
        *EFC = 5.0;
        ...
        if (abc > 0.4) {
          ...
          for (i52 = 0; i52 < loop_ub; i52++) {
            rotated_RD_data[i52] = (RD_data[i52] - abc * (x_data[i52] - SFC)) -
              RD_data[RDS_tmp_tmp];
          }
          ...
          for (n = 0; n < loop_ub; n++) {
            b_rotated_RD_data[n] = rotated_RD_data[i52 + n];
          }
    ...
    if (quick_amp == 1) {
      ...
      i52 = varargin_1->size[0];
      loop_ub = (int)(HTC - 1.0);
      varargin_1->size[0] = (loop_ub + RDS_tmp) - i51;
      emxEnsureCapacity_real_T(varargin_1, i52);
      for (i52 = 0; i52 < loop_ub; i52++) {
        varargin_1->data[i52] = rtMinusInf;
      }

      n = RDS_tmp - i51;
      for (i52 = 0; i52 < n; i52++) {
        varargin_1->data[i52 + loop_ub] = RD_data[i51 + i52];
      }
      ...
      if (varargin_1->size[0] <= 2) {
        ...
        if (RDS_tmp == 0) {
          ...
          i51 = RDS_tmp + 1;
          for (loop_ub = i51; loop_ub <= n; loop_ub++) {
      ...
      c_RD_size[0] = n;
      for (i52 = 0; i52 < n; i52++) {
        rotated_RD_data[i52] = RD_data[i51 + i52];
      }
      ...
      if ((abc > 0.5) && (abc <= 1.5)) {
        ...
        *EFC0 = *EFC;
        ...
        for (i51 = 0; i51 < loop_ub; i51++) {
          scd_fit_data[i51] = cff0_idx_0 * (x_data[i51] - 50.0) * (x_data[i51] -
            50.0) + cff0_idx_1;
        }
        ...
        loop_ub = RD_size[0];
        for (i51 = 0; i51 < loop_ub; i51++) {
          ABSD_data[i51] = RD_data[i51] - scd_fit_data[i51];
        }
        ...
        *SRFU = combineVectorElements(&c_ABSD_data) / (double)loop_ub;
        ...
        if (abc > 1.5) {
          ...
          *EFC0 = 5.0;
          ...
          loop_ub = x_size[0];
          for (i51 = 0; i51 < loop_ub; i51++) {
            scd_fit_data[i51] = 0.0 * (x_data[i51] - 50.0) * (x_data[i51] - 50.0)
              + cff[1];
          }
