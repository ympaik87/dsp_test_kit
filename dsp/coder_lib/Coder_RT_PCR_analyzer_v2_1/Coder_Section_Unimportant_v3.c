  ...
  double FB)
{
  ...
  *result_well = 0.0;
  ...
  loop_ub_tmp = i50 - i49;
  for (i50 = 0; i50 < loop_ub_tmp; i50++) {
    varargin_1_data[i50] = RD_data[(i49 + i50) - 1];
  }

  if (loop_ub_tmp <= 2) {
    if (loop_ub_tmp == 1) {
      f_cff_idx_1 = RD_data[i49 - 1];
    } else if ((RD_data[i49 - 1] > RD_data[i49]) || (rtIsNaN(RD_data[i49 - 1]) &&
                (!rtIsNaN(RD_data[i49])))) {
      f_cff_idx_1 = RD_data[i49];
    } else {
      f_cff_idx_1 = RD_data[i49 - 1];
    }
  } else {
