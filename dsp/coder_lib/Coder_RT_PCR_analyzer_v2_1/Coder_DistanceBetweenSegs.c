  ...
  double SC, double EC)
{
  ...
  for (i5 = 0; i5 < loop_ub; i5++) {
    RD_new_data[i5] = RD_data[i5] - RD_data[(int)SC - 1];
  }
  ...
  i5 = c_y->size[0];
  c_y->size[0] = y->size[1];
  emxEnsureCapacity_real_T(c_y, i5);
  loop_ub = y->size[1];
  for (i5 = 0; i5 < loop_ub; i5++) {
    c_y->data[i5] = y->data[i5];
  }
  ...
  loop_ub = b_y->size[1];
  for (i5 = 0; i5 < loop_ub; i5++) {
    d_y->data[i5] = b_y->data[i5];
  }

  emxFree_real_T(&b_y);
  loop_ub = c_y->size[0];
  for (i5 = 0; i5 < loop_ub; i5++) {
    segment_data[i5] = slope_tmp * (d_y->data[i5] - SC) + RD_new_data[(int)SC -
      1];
  }
  ...
  for (k = 0; k < loop_ub; k++) {
    c_y->data[k] = RD_new_data[i5 + k] - segment_data[k];
  }
