                     ...
                     *diff2, double it[2])
{
  ...
  for (i32 = 0; i32 < nx; i32++) {
    cNdiff_data[i32] = ABSD_data[i32] - fst_val_data[i32];
  }
  ...
  c_diff(sg_data, sg_size, c_data, c_size);
  nx = c_size[1];
  idx = 0;
  lastcycle = c_size[1];
  ii = 0;
  ...
  cp_size[0] = 1;
  cp_size[1] = lastcycle;
  for (i32 = 0; i32 < lastcycle; i32++) {
    cp_data[i32] = ii_data[i32];
  }

  /* ------ 24,25 --> 삭제 /  23,24,25 --> 24만 살림 -------- */
  c_diff(cp_data, cp_size, c_data, c_size);
  nx = c_size[1];
  for (i32 = 0; i32 < nx; i32++) {
    x_data[i32] = (c_data[i32] > 1.0);
  }
  ...
  tmp_data[0] = 0.0;
  for (i32 = 0; i32 < lastcycle; i32++) {
    tmp_data[i32 + 1] = ii_data[i32];
  }

  c_diff(tmp_data, tmp_size, c_data, c_size);
  ...
  for (lastcycle = 0; lastcycle < i32; lastcycle++) {
    ...
    if (EC0 == 0.0) {
      ...
      nx = (signed char)ii - (signed char)i33;
      for (ii = 0; ii <= nx; ii++) {
        ii_data[ii] = (signed char)((signed char)((signed char)i33 + (signed
          char)ii) - 1);
      }

      for (i33 = 0; i33 <= nx; i33++) {
        cp_data[ii_data[i33]] = 0.0;
      }
    } else {
      if ((EC0 != 0.0) && (c_data[lastcycle] > 1.0)) {
        ...
        if (rtIsNaN(b_tmp + 1.0) || rtIsNaN(d_data[lastcycle])) {
          ...
          emxEnsureCapacity_real_T(b_y, i33);
          for (i33 = 0; i33 <= nx; i33++) {
            b_y->data[i33] = (b_tmp + 1.0) + (double)i33;
          }
        } else {
        ...
        i33 = r5->size[0] * r5->size[1];
        r5->size[0] = 1;
        r5->size[1] = y->size[1] + b_y->size[1];
        emxEnsureCapacity_int8_T(r5, i33);
        nx = y->size[1];
        for (i33 = 0; i33 < nx; i33++) {
          r5->data[i33] = (signed char)y->data[i33];
        }

        nx = b_y->size[1];
        for (i33 = 0; i33 < nx; i33++) {
          r5->data[i33 + y->size[1]] = (signed char)b_y->data[i33];
        }

        nx = r5->size[0] * r5->size[1] - 1;
        for (i33 = 0; i33 <= nx; i33++) {
          cp_data[r5->data[i33] - 1] = 0.0;
        }
      }
    }
  }
  ...
  *EC = TC;
  if (lastcycle > 0) {
    ...
    if (cp_data[lastcycle - 1] > cp_data[lastcycle]) {
      ...
      i32 = (int)cp_data[lastcycle - 1] - 1;
      i33 = (int)cp_data[lastcycle];
    }
    ...
    for (i33 = 0; i33 < nx; i33++) {
      y->data[i33] = cNdiff_data[i32 + i33];
    }

    n = nx - 1;
    if (nx <= 2) {
      if (nx == 1) {
        e_ex = cNdiff_data[i32];
        idx = 1;
      } else {
        e_ex = cNdiff_data[i32 + 1];
        if ((cNdiff_data[i32] > e_ex) || (rtIsNaN(cNdiff_data[i32]) && (!rtIsNaN
              (e_ex)))) {
          idx = 2;
        } else {
          e_ex = cNdiff_data[i32];
      ...
      if (idx == 0) {
        ...
        e_ex = cNdiff_data[(i32 + idx) - 1];
        i32 = idx + 1;
        for (ii = i32; ii <= n + 1; ii++) {
          if (e_ex > y->data[ii - 1]) {
            e_ex = y->data[ii - 1];
            idx = ii;
          }
